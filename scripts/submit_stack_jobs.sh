...
for ALGORITHM in ${ALGORITHM_LIST[@]}; do
	for REPLICATE in ${REPLICATE_LIST[@]}; do
		...
		for (( WORKER_NUM = 0; WORKER_NUM < NUM_WORKERS_PER; WORKER_NUM++ )); do
			qsub -v ALGORITHM=$ALGORITHM,REPLICATE=$REPLICATE,INPUT=$INPUT,OUTPUT=$OUTPUT,WORKER_NUM=$WORKER_NUM,NUM_WORKERS_PER=$NUM_WORKERS_PER,SCRIPT_DIR=$SCRIPT_DIR pbs_stack_sdm.sh
		done
	done
done
